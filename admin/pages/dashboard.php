...
if ( is_array( $options['blocking_files'] ) && count( $options['blocking_files'] ) > 0 ) {
	...
	unset( $file );
	echo '
			', __( 'Either delete them (this can be done with the "Fix it" button) or disable XML sitemaps.', 'wordpress-seo' ), '
		</p>';
}
...
if ( $options['theme_description_found'] !== '' ) {
	...
	echo __( 'Your theme contains a meta description, which blocks the SEO plugin from working properly, please delete the following line, or press fix it:', 'wordpress-seo' ), '<br />';
...
?>
	...
	<div id="general" class="wpseotab">
		...
		<p>
			...
			<?php _e( 'We\'ve summarized the most recent changes here.', 'wordpress-seo' ); ?>
		</p>
		<p>
		...
		<p>
			...
			printf( __( 'If you want to restore a site to the default %s settings, press this button.', 'wordpress-seo' ), 'SEO' );
			?>
		</p>
	...
	<div id="security" class="wpseotab">
		...
		$yform->checkbox( 'disableadvanced_meta', __( 'Disable the Advanced part of the SEO meta box', 'wordpress-seo' ) );
		?>
	</div>
<?php
